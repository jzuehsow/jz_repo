...
Change Log:
v2.0 8/30/2017 - Production testing.
v2.1 9/7/2017 - Revised how the users OU is located. 
     9/27/2017 - Corrected error in new OU vailidy test.
...
$items = (Get-ChildItem -Path "[INPUTS PATH]*" -Include *.txt, *.csv).Name
Write-host "Input files must be saved to [INPUTS PATH]" -F Yellow    ;"";""
...
$input = "[INPUTS PATH]\$rmsel"
$log = "[USER PROVISIONING LOGS PATH]\$rmsel"
...
$rmalldn = @()
    ...
    $ou = (($field).Replace("OU=",""))
        ...
        If ($ou -like "????" -or $ou -like "?????" -and $ou -ne "HQHQ" -and $ou -ne "UNET")
        {
...
If (!$rmou){$rmou = "CN=Users,DC=FBI,DC=GOV";$rmout = "Created in default users OU."}
...
$rmtest = $rmtest | sort LastLogonDate -Descending | Select -First 1
    ...
    $i++
        ...
        If ($rmtest.Enabled -eq $false -and $rmtest.SamAccountName -eq $rmuser)
        {
            ...
            Enable-RemoteMailbox -Identity $rmuser -RemoteRoutingAddress "$rmuser@[EXTERNAL DOMAIN]" -DomainController $pdc `
...
Enable-RemoteMailbox -Identity $rmuser -RemoteRoutingAddress "$rmuser@d[EXTERNAL DOMAIN]" -DomainController $pdc `
                     -ErrorAction SilentlyContinue -WarningAction SilentlyContinue > $null 
    $ii = 1
    Do
    {
...
Move-Item $input "[COMPLETED PROVISIONING PATH]" -Force
$items = (Get-ChildItem -Path "[PROVISIONG LOGS PATH]").Name
Foreach ($item in $items)
{
$logfile = "[PROVISIONING LOGS PATH]\$item"
$itemdate = (Get-Item $logfile).lastwritetime
    If ($itemdate -lt $date7){Move-Item $logfile "[USER PROVISIONG PATH]" -Force}
}
$items = (Get-ChildItem -Path "[COMPLETED PROVISIONING PATH]").Name
Foreach ($item in $items)
{
$inputfile = "[COMPLETED PROVISIONING PATH]\$item"
$itemdate = (Get-Item $inputfile).lastwritetime
    If ($itemdate -lt $date7){Remove-Item $inputfile -Recurse -Force}
}
$items = (Get-ChildItem -Path "[INPUT PATH]\*.txt").Name
Foreach ($item in $items)
{
$inputfile = "[INPUT PATH]\$item"
$itemdate = (Get-Item $inputfile).lastwritetime
...
Remove-Variable rm* -Force    ;"";"";""
Write-Host "----------------------------------------------------------------------------------------------------------------" -F Green
Write-Host "GREEN Users Created........." -F Green
Write-Host "----------------------------------------------------------------------------------------------------------------" -F Green
Pause
##############################################################################################################################################################
# End                     
