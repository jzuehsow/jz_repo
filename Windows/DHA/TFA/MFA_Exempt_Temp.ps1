...
################################################################################################################################    
try {
    Add-ADGroupMember $temp_MFA_Ex_group -Server $serv2use -Members $name -ErrorAction SilentlyContinue
}
Catch {
...
write-host "To group:" $temp_MFA_Ex_group
write-host -ForegroundColor Yellow "Confirming Exemption was applied..."
$test1 = Get-ADGroupMember -Identity $temp_MFA_Ex_group -Recursive -Server $serv2use | Select -ExpandProperty sAMAccountName
If ($test1 -contains $name) {
    write-host -ForegroundColor Green "Confirmed... User now has a temporary exemption from using multi-factor authentication."
    } Else {
    write-host -ForegroundColor Red "Unable to confirm that user now has a temporary exemption from using multi-factor authentication."
    }
